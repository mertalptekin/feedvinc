...
{
    public class ProjectPhaseFactory : IMissionService
    {
        private UnitOfWork _services;

        public ProjectPhaseFactory(UnitOfWork _services)
        {
        ...
        {
            var projects = _services.projectRepo.Where(x => selectionIds.Contains(x.ProjectPhaseID)).Select(z => new {
